<script>
import AnalyticsAward from '@/views/dashboard/AnalyticsAward.vue'
import AnalyticsBarCharts from '@/views/dashboard/AnalyticsBarCharts.vue'
import AnalyticsDepositWithdraw from '@/views/dashboard/AnalyticsDepositWithdraw.vue'
import AnalyticsSalesByCountries from '@/views/dashboard/AnalyticsSalesByCountries.vue'
import AnalyticsTotalEarning from '@/views/dashboard/AnalyticsTotalEarning.vue'
import AnalyticsTotalProfitLineCharts from '@/views/dashboard/AnalyticsTotalProfitLineCharts.vue'
import AnalyticsTransactions from '@/views/dashboard/AnalyticsTransactions.vue'
import AnalyticsUserTable from '@/views/dashboard/AnalyticsUserTable.vue'
import AnalyticsWeeklyOverview from '@/views/dashboard/AnalyticsWeeklyOverview.vue'
import CardStatisticsVertical from '@core/components/cards/CardStatisticsVertical.vue'
import { useCorrentistaStore } from '@/stores/CorrentistaStore'
import { ref } from 'vue'

export default {
  components: {
    AnalyticsAward,
    AnalyticsBarCharts,
    AnalyticsDepositWithdraw,
    AnalyticsSalesByCountries,
    AnalyticsTotalEarning,
    AnalyticsTotalProfitLineCharts,
    AnalyticsTransactions,
    AnalyticsUserTable,
    AnalyticsWeeklyOverview,
    CardStatisticsVertical,
  },
  setup() {
    const totalProfit = ref({
      title: 'Total Profit',
      color: 'secondary',
      icon: 'mdi-poll',
      stats: '$25.6k',
      change: 42,
      subtitle: 'Weekly Project',
    })

    const newProject = ref({
      title: 'New Project',
      color: 'primary',
      icon: 'mdi-briefcase-variant-outline',
      stats: '862',
      change: -18,
      subtitle: 'Yearly Project',
    })

    return {
      correntistaId: null,
      totalProfit,
      newProject,
      correntistaStore: useCorrentistaStore(),
    }
  },
  mounted() {
    this.correntistaId = parseInt(localStorage.getItem('pp-correntista-id'))
    this.carregarSaldo()
    this.carregarCorrentista()
    this.carregarContaCorrente()
  },
  methods: {
    carregarSaldo() {
      this.$http.get(`/saldo/${this.correntistaId}`)
        .then(({data}) => this.correntistaStore.setSaldo(data.saldo))
        .catch(x => console.error('Erro ao carregar o saldo', x))
    },
    carregarContaCorrente() {
      this.$http.get(`/conta-corrente-por-correntista/${this.correntistaId}`)
        .then(({data}) => {console.log(data);this.correntistaStore.setContasCorrente(data)})
        .catch(x => console.error('Erro ao carregar o saldo', x))
    },
    carregarCorrentista() {
      this.$http.get(`/correntista/${this.correntistaId}`)
        .then(({data}) => this.correntistaStore.setCorrentista(data))
        .catch(x => console.error('Erro ao carregar o correntista', x))
    }
  }
}
</script>


<template>
  <VRow class="match-height">
    <VCol
      cols="12"
      md="4"
    >
      <AnalyticsAward />
    </VCol>

    <VCol
      cols="12"
      md="8"
    >
      <AnalyticsTransactions />
    </VCol>

    <VCol
      cols="12"
      md="4"
    >
      <AnalyticsWeeklyOverview />
    </VCol>

    <VCol
      cols="12"
      md="4"
    >
      <AnalyticsTotalEarning />
    </VCol>

    <VCol
      cols="12"
      md="4"
    >
      <VRow class="match-height">
        <VCol
          cols="12"
          sm="6"
        >
          <AnalyticsTotalProfitLineCharts />
        </VCol>

        <VCol
          cols="12"
          sm="6"
        >
          <CardStatisticsVertical v-bind="totalProfit" />
        </VCol>

        <VCol
          cols="12"
          sm="6"
        >
          <CardStatisticsVertical v-bind="newProject" />
        </VCol>

        <VCol
          cols="12"
          sm="6"
        >
          <AnalyticsBarCharts />
        </VCol>
      </VRow>
    </VCol>

    <VCol
      cols="12"
      md="4"
    >
      <AnalyticsSalesByCountries />
    </VCol>

    <VCol
      cols="12"
      md="8"
    >
      <AnalyticsDepositWithdraw />
    </VCol>

    <VCol cols="12">
      <AnalyticsUserTable />
    </VCol>
  </VRow>
</template>
